...
HD wallet seed (seed de carteira HD)::
    Seed de carteira HD ou seed raiz é um valor potencialmente curto usado como fundamento para gerar a chave privada mestra e o código de cadeia mestre para uma carteira HD.
...
frase secreta::
    Uma frase secreta é uma string opcional criada pelo usuário que serve como um fator de segurança adicional protegendo o seed, mesmo quando seed é comprometida por um ladrão. Também pode ser usado como uma forma de negação plausível, em que uma frase-senha escolhida leva a uma carteira com uma pequena quantia de fundos usada para desviar um invasor da carteira “real” que contém a maioria dos fundos.
