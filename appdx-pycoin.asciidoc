...
((("pycoin library")))A biblioteca Python http://github.com/richardkiss/pycoin[+pycoin+], originalmente escrito e mantido por Richard Kiss, é uma biblioteca baseada em Python que suporta a manipulação de chaves e transações bitcoin, até mesmo suportando a linguagem de script o suficiente para lidar adequadamente com transações fora do padrão.

A biblioteca pycoin suporta Python 2 (2.7.x) e Python 3 (3.3 e posterior) e vem com alguns utilitários de linha de comando úteis, +ku+ e +tx+.

=== Key Utility (KU) (Utilitário de Chave)

((("key utility (ku)", id="keyutil17")))O utilitário de linha de comando +ku+ ("utilitário de chave") é um canivete suíço para manipular chaves. Suporta chaves BIP-32, WIF e endereços (bitcoin e moedas alternativas). A seguir estão alguns exemplos.

Crie uma chave BIP-32 usando as fontes de entropia padrão de GPG e _/dev/random_:
...
Crie uma chave BIP-32 a partir de uma senha longa:

[WARNING]
====
A frase secreta neste exemplo é muito fácil de adivinhar.
====
...
Gere os endereços correspondentes:
...
Verifique se funciona escolhendo uma string BIP32 (aquela correspondente à subchave 0/3):
...
==== Transaction Utility (TX) (Utilitário de Transação)

((("transaction utility (TX)", id="TX17")))
O utilitário de linha de comando +tx+ exibirá transações em formato legível por humanos, buscará transações básicas do cache de transações do pycoin ou de serviços da web (blockchain.info, blockcypher.com, blockr.io e chain.so são atualmente suportados), mesclar transações , adicionar ou excluir entradas ou saídas e assinar transações.
...
Ops! Não temos serviços da web configurados. Vamos fazer isso agora:
...
Isso não é feito automaticamente, portanto, uma ferramenta de linha de comando não vaza informações potencialmente privadas sobre quais transações você está interessado em um site de terceiros. Se você não se importar, você pode colocar essas linhas em seu _.profile_.
...
A linha final aparece porque para validar as assinaturas das transações, você tecnicamente precisa das transações de origem. Então, vamos adicionar +-a+ para aumentar as transações com informações de origem:
...
((("", startref="TX17")))Agora, vamos examinar as saídas não gastas para um endereço específico (UTXO). No bloco 1, vemos uma transação coinbase para +12c6DSiU4Rq3P4ZxziKxzrL5LmMBrzjrJX+. Vamos usar +fetch_unspent+ para encontrar todas as moedas neste endereço:
